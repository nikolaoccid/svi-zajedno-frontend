...
import {deleteUserRequests} from "../../../api/api/api.ts";
...
export function ActivityView() {
    ...
    const deleteRequest = async (requestId) => {
        console.log('about to delete request', requestId)
        try {
            await deleteUserRequests(requestId);
    ...
    const getRequestColor = (status: 'approved' | 'pending' | 'rejected') => (status === 'approved' ? 'green' : status === 'rejected' ? 'red' : 'yellow');
            ...
            <ProfileContainer>
                <Row>
                    <HeaderContainer>
                        ...
                        <Status
                            status={(studentOnActivity && (studentOnActivity as any).activityStatus) ?? 'inactive'}></Status>
                    </HeaderContainer>
                    <MdEdit
                        size={18}
                        color={'#00193f'}
                                ...
                                {activityRequests
                                    ...
                                    .map((request) => (
                                        <TableRow key={request.id}>
                                            <Icon onClick={() => editRequest(request.id)}>
                                                <GoDotFill size={18}
                                                           color={getRequestColor(request.userRequestStatus)}/>
                                            </Icon>
                                            <TableData
                                                onClick={() => editRequest(request.id)}>{request.userRequestTitle}</TableData>
                                            <TableData
                                                onClick={() => editRequest(request.id)}>{request.userRequestStoreInfo}</TableData>
                                            <TableData
                                                onClick={() => editRequest(request.id)}>{request.userRequestDescription}</TableData>
                                            <TableData
                                                onClick={() => editRequest(request.id)}>{request.userRequestQuantity}</TableData>
                                            <TableData
                                                onClick={() => editRequest(request.id)}>{request.userRequestCostPerUnit}</TableData>
                                            <Icon>
                                                <MdDelete
                                                    size={18}
                                                    color={'#00193f'}
